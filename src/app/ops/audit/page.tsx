...
export default async function AuditPage({
  ...
  const roles = await sql<{ actor_role: string; n: string }[]>`
    SELECT actor_role, COUNT(*)::text n FROM app.audit_log GROUP BY actor_role ORDER BY actor_role`;
  ...
  const rows = role
    ? await sql<{ id: string; actor: string; actor_role: string; event: string; entity_type: string | null; entity_id: string | null; reason: string | null; detail: Record<string, unknown>; created_at: Date }[]>`
        SELECT * FROM app.audit_log WHERE actor_role = ${role} ORDER BY created_at DESC, id DESC LIMIT 200`
    : await sql<{ id: string; actor: string; actor_role: string; event: string; entity_type: string | null; entity_id: string | null; reason: string | null; detail: Record<string, unknown>; created_at: Date }[]>`
        SELECT * FROM app.audit_log ORDER BY created_at DESC, id DESC LIMIT 200`;
