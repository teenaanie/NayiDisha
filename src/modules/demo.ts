...
export async function resetDemo(confirmation:string){
 ...
 // CASCADE, not a bare TRUNCATE: tables added after this snapshot was captured
 // (voice turns, screening calls, job suggestions, reminders, workflow issues,
 // credit requests, script runs) hold foreign keys into it, and Postgres
 // refuses to truncate a referenced table unless the referencing one goes too.
 // Clearing them is also what a demo reset means — they are derived records,
 // regenerated by the journey, never part of the canonical dataset.
